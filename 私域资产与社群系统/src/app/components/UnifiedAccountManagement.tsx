...
export default function UnifiedAccountManagement({ initialCategory = "all", initialSubtype }: UnifiedAccountManagementProps = {}) {
  ...
  const toggleSubtype = (key: string) => {
    ...
    setSelectedSubtypeKeys(current => {
      const next = isReset ? [key] : current.includes(key) ? current.filter(item => item !== key) : [...current.filter(item => !item.startsWith("all-")), key];
  ...
  const assetNav = (mobile = false) => (
    <div className={`${mobile ? "flex" : "hidden xl:flex"} items-center flex-shrink-0 overflow-x-auto`} role="tablist" aria-label="账号资产类型">
      {CATEGORY_TABS.map((tab, index) => {
        ...
        const Icon = tab.icon;
        return <button key={tab.key} type="button" role="tab" aria-selected={active} title={tab.hint} onClick={() => selectCategory(tab.key)} className="flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-semibold transition-all whitespace-nowrap" style={{ background: active ? "#0d0d0d" : "transparent", color: active ? "#ccff00" : "#888", borderRight: index < CATEGORY_TABS.length - 1 ? "1px solid rgba(0,0,0,0.06)" : "none", fontFamily: "monospace" }}><Icon size={13} />{tab.label}</button>;
      })}
    </div>
  );

  return (
    <div className="h-full min-h-0 flex flex-col" style={{ background: "#fafafa" }}>
      <div className="flex items-center justify-between gap-4 px-6 py-2.5 flex-shrink-0" style={{ background: "#fff", borderBottom: "1px solid rgba(0,0,0,0.06)" }}>
        <div className="flex items-center gap-4 min-w-0">
          <Building2 size={14} style={{ color: "#888" }} />
          <div className="min-w-0">
            <div className="text-xs font-semibold" style={{ color: "#111", fontFamily: "monospace" }}>账号资产中心</div>
            <div className="text-[10px] truncate" style={{ color: "#888", fontFamily: "monospace" }}>统一台账 · 五阶段生命周期 · 三种查看维度</div>
          </div>
        </div>
        <div className="flex items-center flex-shrink-0" role="tablist" aria-label="查看维度">
          {([
            ...
            const Icon = tab.icon;
            return <button key={tab.key} type="button" role="tab" aria-selected={active} title={tab.hint} onClick={() => setViewDimension(tab.key)} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold transition-all whitespace-nowrap" style={{ background: active ? "#0d0d0d" : "transparent", color: active ? "#ccff00" : "#888", borderRight: index < 2 ? "1px solid rgba(0,0,0,0.06)" : "none", fontFamily: "monospace" }}><Icon size={13} />{tab.label}</button>;
          })}
        </div>
      </div>

      <div className="flex items-center justify-between gap-4 px-6 py-2 flex-shrink-0 overflow-x-auto" style={{ background: "#fff", borderBottom: "1px solid rgba(0,0,0,0.06)" }}>
        {assetNav(true)}
