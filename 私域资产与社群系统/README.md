...
7. 建立项目群类型规则共享数据源：项目配置中的规则会驱动微信群创建，地区和会员身份支持多选编辑，群类型代码与编号保持系统生成。
8. 新建微信群支持轮巡/统一分配，按地区、微信号建号时间和客服继承关系自动归属；生成群会回写群库及微信号管理的群位统计。
9. 新建微信号改为二维码授权优先：授权前表单锁定，授权后自动回填微信资料并解锁绑定人、安全、紧急联系人和归属信息。

## 当前版本交付

- 预览地址：`http://127.0.0.1:5184/?view=pc&module=wechat`
- 项目配置：`?view=pc&module=ecosystem`，进入 `Platform / Project` 后打开“群类型规则”。
- 社群管理：`?view=pc&module=community`，点击“新建微信群”验证规则驱动的建群流程。
- 微信管理：点击“新建微信号”，先完成左侧二维码授权演示，再继续填写右侧账号资料。
- 验证命令：`npm run build`、`git diff --check`。

本版本的二维码授权使用前端演示回调按钮模拟扫码成功；接入真实微信登录时，将该回调替换为后端授权状态轮询/WebSocket，并保留当前表单回填契约。
